    ...
    public function dataTable($query)
    {
        return DataTables::of($query)
            ...
            ->addColumn('action', function ($data){
                ...
                $access = "<button type='button' class='btn btn-outline-info btn-sm tooltips access m-1' title='Edit Privilage' data-bs-toggle='tooltip' data-bs-placement='top' data-id=".$data->id."><i class='fas fa-user-shield'></i></button>";
                $action = '';
                if (Gate::check('rolesetting-U')) {
                    $action .= $edit;
                    $action .= $access;
                }
                if (Gate::check('rolesetting-D')) {
                    $action .= $delete;
                }
                return $action;
            })
            ->rawColumns(['name', 'desc', 'action']);
    }

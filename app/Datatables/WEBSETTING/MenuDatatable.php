...
class MenuDataTable extends DataTable
{
    ...
    {
        $this->menuData = Menu::get()->pluck('name', 'id')->toArray();
    }
    ...
    public function dataTable($query)
    {
        return DataTables::of($query)
            ...
            ->addColumn('parent_id', function ($data) {
                return $data->parent_id == 0 ? 'Parent' : $this->menuData[$data->parent_id];
            })
            ...
            ->addColumn('action', function ($data) {
                ...
                if (Gate::check('menusetting-U')) {
                    $action .= $edit;
                }
            
                if (Gate::check('menusetting-D')) {
                    $action .= $delete;
                }
            
                return $action;
            })
            ->rawColumns(['parent', 'icon', 'action', 'status', 'url']);
    }
    ...
    protected function getColumns()
    {
        return [
            ...
            Column::make('name')->title('Menu Name'),
            Column::make('url')->title('URL'),
            Column::make('icon')->title('Icon')->orderable(false)->searchable(false),
            Column::make('parent_id')->title('Parent')->searchable(true), // Set searchable to true
            Column::make('type')->title('Type'),
            Column::make('permission')->title('Permission'),
            Column::make('status')->title('Status')->searchable(true), // Set searchable to true
